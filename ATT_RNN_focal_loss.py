...
VALIDATION_SPLIT = 0.05  # validation ratio


gammas  = np.array([2.5 for i in range(19)])
def focal_loss(y_true, y_pred):
    losses = y_true * K.log(y_pred + K.epsilon()) * K.pow(1 - y_pred, gammas)
    losses = - K.mean(losses, axis=-1)
    return losses
...
if __name__ == "__main__":
    ...
    model.compile(loss=focal_loss,
                ...
                metrics=['accuracy'])
    early_stop = EarlyStopping(monitor='val_loss' if VALIDATION_SPLIT > 0 else "loss", patience=30, mode='min')
